...
class InvoiceController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        DB::transaction(function () use ($request, $invoiceNo) {
            ...
            foreach ($request->barang_id ?? [] as $i => $id) {
                ...
                $barangMap[$id]['total'] = $barangMap[$id]['qty'] * $barangMap[$id]['harga'];
            }
            ...
            $paymentAwal = min($paymentAwal, $grandTotal);
            ...
            $invoice->sisa          = $sisa;
            $invoice->status_bayar  = $statusBayar;
    ...
    public function update(Request $request, Invoice $invoice)
    {
        DB::transaction(function () use ($request, $invoice) {
            ...
            // Barang lama yang dihapus → kembalikan stok
            ...
            if ($request->status_bayar === 'sudah') {
                // User pilih Lunas → payment_awal = grand total
                $paymentAwal = $grandTotal;
            } else {
                // User pilih Belum Lunas → ambil dari input, strip format rupiah
                $rawPayment  = preg_replace('/[^0-9]/', '', $request->payment_awal ?? '0');
                $paymentAwal = (int) $rawPayment;
                // Clamp — tidak boleh melebihi grand total
                $paymentAwal = min($paymentAwal, $grandTotal);
            }

            /* ================= SISA & STATUS ================= */
            // Hitung ulang dari nol: grandTotal - DP - semua cicilan
            $totalCicilan  = $invoice->payments()->sum('jumlah');
            $totalTerbayar = $paymentAwal + $totalCicilan;
            $sisa          = max(0, $grandTotal - $totalTerbayar);
            $statusBayar   = $sisa == 0 ? 'sudah' : 'belum';
    ...
    public function outstanding()
    {
        ...
        $totalAll = Invoice::where('sisa', '>', 0)->sum('grand_total');

        $totalOutstanding = Invoice::where('sisa', '>', 0)->sum('sisa');
    ...
    public function cicilanStore(Request $request, Invoice $invoice)
    {
        $request->validate([
            ...
            'metode'        => 'required|in:cash,bca,mandiri',
        ]);
